/**
 * Theme-aware color loader.
 *
 * Purpose
 * -------
 * Reads the user's active VS Code color theme and maps its TextMate token colors
 * to the color keys used by lib/colors.js. This lets the extension blend with
 * whatever theme the user has installed rather than always showing hard-coded hex
 * values.
 *
 * Public API
 * ----------
 * loadColors(fallback)
 *   Accepts the static colors from lib/colors.js as a fallback.
 *   Returns a colors object with the same keys, where each value is either
 *   resolved from the active theme or the corresponding fallback color.
 *
 * How it works
 * ------------
 * 1. Reads workbench.colorTheme from VS Code configuration.
 * 2. Locates the theme's JSON/JSONC file by scanning all installed extensions.
 * 3. Follows "include" chains so inherited themes are resolved correctly.
 * 4. Applies TextMate longest-prefix scope matching (the same rule editors use)
 *    to pick the best foreground color for each semantic role.
 *
 * Note: loadColors is intentionally not called at startup in the current build —
 * lib/colors.js provides hand-tuned values that override theme colors. To enable
 * theme-aware coloring, call loadColors(colors) in bin/index.js instead of using
